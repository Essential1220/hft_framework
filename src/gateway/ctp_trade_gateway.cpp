// ============================================
// ctp_trade_gateway.cpp - CTP trade gateway implementation (CTP 交易网关实现)
// ============================================
...
std::string ctp_error_text(CThostFtdcRspInfoField* pRspInfo) {
    if (!pRspInfo) {
        return "CTP: unknown error";
    }
...
// CTP 查询限流器实现：CTP 柜台对查询请求有严格的 1 秒间隔限制，
// 违反会返回 -3 (THOST_FTDC_ERR_FlowControl)。此方法在每次查询前调用，
// 确保两次查询之间至少间隔 kQueryIntervalMs（1100ms，含 100ms 安全余量）。
// 使用 mutex 串行化并发查询请求。
void CtpTradeGateway::throttle_query() {
    ...
    if (elapsed < kQueryIntervalMs) {
        const auto wait_ms = kQueryIntervalMs - elapsed;
...
void CtpTradeGateway::init(const Config& config, const std::string& section,
    ...
    app_id_ = config.get_string(section, "AppID");
    auth_code_ = crypto::decrypt_config_value(config.get_string(section, "AuthCode"));
    std::string front = config.get_string(section, "TradeFront");
    if (front.empty()) {
        LOG_ERROR("交易网关前置地址为空, section=" + section);
        login_failed_ = true;
        return;
    }
    logged_in_ = false;
    ...
    if (!api_) {
        LOG_ERROR("交易网关创建API失败, section=" + section);
        login_failed_ = true;
        return;
    }
    api_->RegisterSpi(this);
    
    // 订阅私有流和公共流（QUICK 表示从本交易日开始接收）
    ...
    LOG_INFO("交易网关连接前置: " + front);
    api_->Init(); // 启动连接
}

void CtpTradeGateway::stop() {
    if (api_) {
        ...
        LOG_INFO("trade gateway stopped");
    }
}

// 等待登录完成
bool CtpTradeGateway::wait_for_login(int timeout_sec) {
...
int CtpTradeGateway::send_order(const OrderRequest& req, const std::string& order_ref) {
    if (!api_) return -1; // gateway 已关闭
    ...
    switch (req.offset) {
        ...
        default:
            LOG_ERROR("send_order: unknown offset value=" + std::to_string(static_cast<int>(req.offset)) +
                      " ref=" + order_ref);
            return -1;
    }

    field.CombHedgeFlag[0] = THOST_FTDC_HF_Speculation; // 投机
    if (req.price_type != OrderRequest::PriceType::Market &&
        ...
        field.VolumeCondition = THOST_FTDC_VC_AV;           // 任何数量
    }
    ...
    if (ret != 0) {
        LOG_ERROR("交易网关报单失败, ret=" + std::to_string(ret) +
                  " ref=" + order_ref);
    } else {
        LOG_INFO("交易网关报单已发送 ref=" + order_ref +
...
int CtpTradeGateway::cancel_order(const std::string& instrument_id, const std::string& exchange_id,
    ...
    field.ActionFlag = THOST_FTDC_AF_Delete; // 删除（撤单）

    const int request_id = request_id_.fetch_add(1, std::memory_order_relaxed) + 1;
    const int ret = api_->ReqOrderAction(&field, request_id);
    if (ret != 0) {
        LOG_ERROR("交易网关撤单失败, ret=" + std::to_string(ret) +
                  " ref=" + order_ref);
    } else {
        LOG_INFO("交易网关撤单已发送 ref=" + order_ref);
    }
    return ret;
}

// 查询资金账户
int CtpTradeGateway::query_account() {
    throttle_query();
...
int CtpTradeGateway::query_position(const std::string& instrument_id) {
    throttle_query();
    const int request_id = request_id_.fetch_add(1, std::memory_order_relaxed) + 1;
    {
...
// 查询活动委托
int CtpTradeGateway::query_active_orders() {
    throttle_query();
    const int request_id = request_id_.fetch_add(1, std::memory_order_relaxed) + 1;
    {
...
int CtpTradeGateway::query_instrument_rates(const std::string& instrument_id, const std::string& exchange_id) {
    if (!api_ || !logged_in_) {
        return -1;
    }
    if (instrument_id.empty()) {
    ...
    CThostFtdcQryInstrumentMarginRateField margin_field{};
    ...
    if (margin_ret != 0) {
        LOG_WARN("query margin rate failed ret=" + std::to_string(margin_ret) + " instrument=" + instrument_id);
    }

    // margin 和 commission 是两次独立的 CTP 查询请求，中间也需要限流
    throttle_query();

    CThostFtdcQryInstrumentCommissionRateField commission_field{};
    ...
    if (commission_ret != 0) {
        LOG_WARN("query commission rate failed ret=" + std::to_string(commission_ret) + " instrument=" + instrument_id);
    }

    return margin_ret != 0 ? margin_ret : commission_ret;
}
std::vector<std::string> CtpTradeGateway::query_instruments(int timeout_sec) {
    if (!api_ || !logged_in_) {
        LOG_WARN("交易网关查询合约列表失败：网关未登录，account=" + account_id_);
        return {};
    }
    throttle_query();
    ...
    if (ret != 0) {
        LOG_ERROR("交易网关查询合约列表请求失败, ret=" + std::to_string(ret) + " account=" + account_id_);
        std::lock_guard<std::mutex> lock(pending_instruments_mtx_);
    ...
    if (!ready) {
        LOG_WARN("交易网关查询合约列表超时, account=" + account_id_);
...
void CtpTradeGateway::OnFrontConnected() {
    LOG_INFO("trade gateway front connected, authenticating");
    req_authenticate();
}

void CtpTradeGateway::OnFrontDisconnected(int nReason) {
    logged_in_ = false;
    // nReason - CTP 定义的断开原因码，常见值：
    //   0x1001 = 网络读失败
    //   0x1002 = 网络写失败
    //   0x2001 = 接收心跳超时
    //   0x2002 = 发送心跳失败
    ...
    LOG_WARN("交易网关已断开, reason=" + std::string(hex_buf) + " account=" + account_id_);

    if (engine_) {
        engine_->on_gateway_error(account_id_, OrderRejectReason::GatewayDisconnected,
                                  "CTP trade front disconnected reason=" + std::string(hex_buf));
    }
}

void CtpTradeGateway::OnRspAuthenticate(CThostFtdcRspAuthenticateField*,
                                        CThostFtdcRspInfoField* pRspInfo,
                                        int, bool) {
    if (is_error_rsp(pRspInfo)) {
        report_gateway_error(OrderRejectReason::GatewayLoginFailed,
                             "CTP trade authenticate failed " + ctp_error_text(pRspInfo));
        LOG_ERROR("trade gateway authentication failed");
        return;
    }
    LOG_INFO("trade gateway authenticated, logging in");
    req_login();
}

void CtpTradeGateway::OnRspUserLogin(CThostFtdcRspUserLoginField* pRspUserLogin,
                                     CThostFtdcRspInfoField* pRspInfo,
                                     int, bool) {
    if (is_error_rsp(pRspInfo)) {
        report_gateway_error(OrderRejectReason::GatewayLoginFailed,
                             "CTP trade login failed " + ctp_error_text(pRspInfo));
        LOG_ERROR("trade gateway login failed");
        return;
    }

    if (!pRspUserLogin) {
        report_gateway_error(OrderRejectReason::GatewayLoginFailed,
                             "CTP trade login returned null response");
        LOG_ERROR("trade gateway login failed: pRspUserLogin is null");
        return;
    }
    ...
    LOG_INFO("交易网关登录成功: front_id=" + std::to_string(front_id_) +
...
void CtpTradeGateway::OnRspSettlementInfoConfirm(CThostFtdcSettlementInfoConfirmField*,
                                                 CThostFtdcRspInfoField* pRspInfo,
                                                 int, bool) {
    if (is_error_rsp(pRspInfo)) {
        report_gateway_error(OrderRejectReason::GatewayLoginFailed,
                             "CTP settlement confirm failed " + ctp_error_text(pRspInfo));
        LOG_ERROR("trade gateway settlement confirm failed");
        return;
    }
    ...
    if (!initial_login_done_) {
        ...
        LOG_INFO("交易网关结算确认完成，交易已就绪");
    } else {
        LOG_INFO("交易网关重登录完成 account=" + account_id_);
        if (engine_) {
...
void CtpTradeGateway::OnRspOrderInsert(CThostFtdcInputOrderField* pInputOrder,
                                       CThostFtdcRspInfoField* pRspInfo,
                                       int, bool) {
...
void CtpTradeGateway::OnRspOrderAction(CThostFtdcInputOrderActionField* pInputOrderAction,
                                       CThostFtdcRspInfoField* pRspInfo,
                                       int, bool) {
    if (is_error_rsp(pRspInfo) && pInputOrderAction) {
        const std::string reason = gbk_to_utf8(pRspInfo ? pRspInfo->ErrorMsg : "Unknown error");
        LOG_ERROR("交易网关撤单被拒绝 ref=" + std::string(pInputOrderAction->OrderRef) +
                  " account=" + account_id_ +
                  " reason=" + reason);
        if (engine_) {
...
void CtpTradeGateway::OnRtnOrder(CThostFtdcOrderField* pOrder) {
    ...
    LOG_INFO("OnRtnOrder ENTER ref=" + std::string(pOrder->OrderRef) +
             " status=" + std::to_string(static_cast<int>(pOrder->OrderStatus)) +
             " inst=" + std::string(pOrder->InstrumentID));
    ...
    switch (pOrder->OrderStatus) {
        ...
        case THOST_FTDC_OST_Unknown:                info.status = OrderStatus::Pending; break;
        default:
            info.status = OrderStatus::Error;
            LOG_WARN("unmapped CTP OrderStatus=" + std::to_string(static_cast<int>(pOrder->OrderStatus)) +
...
void CtpTradeGateway::OnRspQryTradingAccount(CThostFtdcTradingAccountField* pTradingAccount,
                                             CThostFtdcRspInfoField* pRspInfo,
                                             int, bool bIsLast) {
...
void CtpTradeGateway::OnRspQryInvestorPosition(CThostFtdcInvestorPositionField* pPos,
                                               CThostFtdcRspInfoField* pRspInfo,
...
void CtpTradeGateway::OnRspQryOrder(CThostFtdcOrderField* pOrder,
                                    CThostFtdcRspInfoField* pRspInfo,
                                    int nRequestID, bool bIsLast) {
    if (is_error_rsp(pRspInfo)) return;
    if (!engine_) return;
    ...
    if (pOrder) {
        ...
        switch (pOrder->OrderStatus) {
            ...
            default:
                info.status = OrderStatus::Error;
                LOG_WARN("unmapped CTP OrderStatus in QryOrder: " +
...
void CtpTradeGateway::OnRspError(CThostFtdcRspInfoField* pRspInfo, int, bool) {
    is_error_rsp(pRspInfo);
}

void CtpTradeGateway::req_authenticate() {
    CThostFtdcReqAuthenticateField field{};
    std::strncpy(field.BrokerID, broker_id_.c_str(), sizeof(field.BrokerID) - 1);
    std::strncpy(field.UserID, user_id_.c_str(), sizeof(field.UserID) - 1);
    std::strncpy(field.AppID, app_id_.c_str(), sizeof(field.AppID) - 1);
    std::strncpy(field.AuthCode, auth_code_.c_str(), sizeof(field.AuthCode) - 1);

    const int request_id = request_id_.fetch_add(1, std::memory_order_relaxed) + 1;
    const int ret = api_->ReqAuthenticate(&field, request_id);
    if (ret != 0) {
        report_gateway_error(OrderRejectReason::GatewayLoginFailed,
                             "CTP trade authenticate request failed ret=" + std::to_string(ret));
        LOG_ERROR("交易网关发送认证请求失败 ret=" + std::to_string(ret));
    }
}

void CtpTradeGateway::OnRspQryInstrumentMarginRate(CThostFtdcInstrumentMarginRateField* pRate,
                                                   CThostFtdcRspInfoField* pRspInfo,
                                                   int, bool) {
    ...
    LOG_INFO("instrument margin rate updated: " + spec.instrument_id);
}

void CtpTradeGateway::OnRspQryInstrumentCommissionRate(CThostFtdcInstrumentCommissionRateField* pRate,
                                                       CThostFtdcRspInfoField* pRspInfo,
                                                       int, bool) {
    if (is_error_rsp(pRspInfo) || !pRate || !engine_) {
        return;
    ...
    if (spec.exchange_id.empty()) spec.exchange_id = pRate->ExchangeID;
    engine_->on_instrument_spec_update(spec);
    LOG_INFO("instrument commission rate updated: " + spec.instrument_id);
}
void CtpTradeGateway::OnRspQryInstrument(CThostFtdcInstrumentField* pInstrument,
                                         CThostFtdcRspInfoField* pRspInfo,
                                         int nRequestID, bool bIsLast) {
    {
        std::lock_guard<std::mutex> lock(pending_instruments_mtx_);
    ...
    if (bIsLast || is_error_rsp(pRspInfo)) {
        pending_instruments_cv_.notify_all();
        if (!is_error_rsp(pRspInfo)) {
            LOG_INFO("交易网关合约列表查询完成, account=" + account_id_ +
                     " count=" + std::to_string(pending_instruments_.size()));
        }
    }
}

void CtpTradeGateway::req_login() {
    ...
    if (ret != 0) {
        report_gateway_error(OrderRejectReason::GatewayLoginFailed,
                             "CTP trade login request failed ret=" + std::to_string(ret));
        LOG_ERROR("交易网关发送登录请求失败 ret=" + std::to_string(ret));
    }
}

void CtpTradeGateway::req_settlement_confirm() {
    ...
    if (ret != 0) {
        report_gateway_error(OrderRejectReason::GatewayLoginFailed,
                             "CTP settlement confirm request failed ret=" + std::to_string(ret));
        LOG_ERROR("交易网关发送结算确认请求失败 ret=" + std::to_string(ret));
    }
}

void CtpTradeGateway::report_gateway_error(OrderRejectReason reason, const std::string& message) {
    login_failed_ = true;
    login_cv_.notify_all();
    if (engine_) {
...
bool CtpTradeGateway::is_error_rsp(CThostFtdcRspInfoField* pRspInfo) {
    if (pRspInfo && pRspInfo->ErrorID != 0) {
        LOG_ERROR("交易网关错误: [" + std::to_string(pRspInfo->ErrorID) + "] " +
